...
def generate_invoice_pdf(data):
    ...
    # Manual total overrides (Fully manual): whatever Jasmine types prints exactly;
    # blank fields fall back to the computed values.
    ...
    # Fully refundable pet deposit — deliberately kept OUT of subtotal/tax/grand above and
    # billed via its own separate Stripe payment link, so it never reads as taxable revenue.
    pet_deposit = data.get('pet_deposit') or {}
    ...
    # Refundable pet deposit — its own clearly separated block, never part of the taxed
    # total above, and billed via a distinct payment link the client receives alongside
    # this invoice.
    if pet_deposit.get('enabled'):
        ...
        if dep_url:
            cv.setFont('Helvetica', 7.5); cv.setFillColor(colors.HexColor('#1a73e8'))
            cv.drawString(ML, y, 'Pay deposit: ' + dep_url[:80]); y -= 10
        y -= 4
        cv.setStrokeColor(LGRAY); cv.setLineWidth(0.5)
        cv.rect(ML - 8, y, CW + 16, box_top - y + 6, stroke=1, fill=0)
        y -= 14
